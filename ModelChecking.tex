...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[main node] (Reset) {Reset};
  \node[main node] (Coin) [right of=Reset] {Coin};
  \node[main node] (Coffee) [right of=Coin] {Coffee};
  \node[main node] (Change) [right of=Coffee] {Change};

  \path[every node/.style={font=\sffamily\small}]
    (Reset) edge node [] {Entercoin} (Coin)
       
    (Coin) edge node [] {PushCof} (Coffee)
       
    (Coffee) edge node [] {PushChange} (Change)
    
    (Change) edge [bend left] node [] {} (Reset)
        
    (Coffee) edge [bend left] node [above] {GotCof} (Reset);
